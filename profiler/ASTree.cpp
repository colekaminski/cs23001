...
void AST::swap(AST& rhs) {
	...
	//Creating TMP
	AST newAST;
	newAST.child = rhs.child;
	newAST.closeTag = rhs.closeTag;
	newAST.nodeType = rhs.nodeType;
	newAST.tag = rhs.tag;
	newAST.text = rhs.text;

	//Current assign to rhs
	rhs.child = child;
	...
	//TMP to current
	child = newAST.child;
	closeTag = newAST.closeTag;
	nodeType = newAST.nodeType;
	tag = newAST.tag;
	text = newAST.text;
}
...
void AST::mainHeader(const std::vector<std::string>& profileName,
	...
	while ((*ip)->tag != "function") {
		ip++;
	}

	std::string profileString = "#include \"profile.hpp\"";

	child.insert(ip, new AST(token, profileString));

	child.insert(ip, new AST(whitespace, "\n"));

	for (int i = 0; i < profileName.size(); ++i) {
		std::string profilerNames = "profile " + profileName[i] + "(\"" + fileName[i] + "\");";
		child.insert(ip, new AST(token, profilerNames));
		child.insert(ip, new AST(whitespace, "\n"));
	}
...
void AST::fileHeader(const std::string& profileName) {
	...
	std::string profilerNames = "extern profile " + profileName + ";";
	child.insert(ip, new AST(token, profilerNames));
	child.insert(ip, new AST(whitespace, "\n"));
	child.insert(ip, new AST(whitespace, "\n"));
...
void AST::mainReport(const std::vector<std::string>& profileName) {
	...
	while (ip != child.end()) {
		if ((*ip)->tag == "function") {
			...
			std::list<AST*>::iterator childOfFunc = (*ip)->child.begin();
			while (childOfFunc != (*ip)->child.end()) {
				if ((*childOfFunc)->tag == "name") {
					name = (*childOfFunc)->getName();
					break;
				}
				childOfFunc++;
			}
			if (name == "main") {
				AST* mainBlock = (*ip)->getChild("block")->getChild("block_content");
				std::list<AST*>::iterator childOfMB = --(mainBlock->child.end());

				while (childOfMB != mainBlock->child.begin()) {
					if ((*childOfMB)->tag == "return") {
						for (int i = 0; i < profileName.size(); ++i) {
							std::string reportOutput = "std::cout << " + profileName[i] + " << std::endl;";
							mainBlock->child.insert(childOfMB, new AST(token, reportOutput));
							mainBlock->child.insert(childOfMB, new AST(whitespace, "\n"));
						}
						mainBlock->child.insert(childOfMB, new AST(whitespace, "\n"));
						mainBlock->child.insert(childOfMB, new AST(whitespace, "\t"));
						return;
...
void AST::functionCount(const std::string& profileName) {
	...
	while (ip != child.end()) {
		...
		if ((*ip)->tag == "function" || (*ip)->tag == "destructor" || (*ip)->tag == "constructor") {
			std::string name;
			AST* function = (*ip)->getChild("block")->getChild("block_content");
			...
			std::string profileString = " " + profileName + ".count(__LINE__, \"" + name + "\");";
			function->child.insert(function->child.begin(), new AST(token, profileString));
		}
		ip++;
	}
...
void AST::lineCount(const std::string& profileName) {
	...
	while (ip != child.end()) {
		...
		if ((*ip)->nodeType == category) {
			if ((*ip)->tag == "expr_stmt") {
				std::string profileString = " " + profileName + ".count(__LINE__);";
				(*ip)->child.insert((*ip)->child.end(), new AST(token, profileString));
			}
			else if (!isStopTag((*ip)->tag)) {
				(*ip)->lineCount(profileName);
			}
		}
		ip++;
	}
